...
class CheckoutController extends Controller
{
    ...
    {
        if ($wisata->isRegistered) {
            session()->flash('error', "You already registered as {$wisata->title} visitor.");
    ...
    public function store(StoreRequest $request, Wisata $wisata)
    {
        try {
            DB::beginTransaction();
    ...
    public function getSnapRedirect(Checkout $checkout){
        ...
        $price = $checkout->wisata->price * 1000;
        ...
        $item_details[]      = [
            ...
            'price'             => $price,
            'quantity'          => 1,
            'name'              => "Payment for {$checkout->wisata->title} ticket.",
        ];
